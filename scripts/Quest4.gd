...
func _place_npcs() -> void:
	# "Dan will be a serpent by the roadside, a viper along the path" – Genesis 49:17
	var elder_name: String = _tribe_data.get("elder", "Elder Shuham") as String
	_build_npc("elder_shuham", Vector3(-60, 0, 40))
	show_dialogue(
		[
			{
				"name": elder_name,
			...
			{ "name": "You", "text": "Please, Elder — what must I do?" },
			{
				"name": elder_name,
	...
	# Scout lookout NPC — east plateau
	_build_npc("scout_adina", Vector3(320, 0, -280))
...
func _after_soar() -> void:
	...
	show_dialogue(
		[
			...
			{
				"name": elder,
				"text": "Find and tap all four hidden marks in order. Deborah's training will help.",
