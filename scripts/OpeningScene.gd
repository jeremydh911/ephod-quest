...
func _build_terrain() -> void:
	...
	for i in range(8):
		...
		_add_sphere(Vector3(px, 0.8, pz), 0.8 + randf_range(0, 0.6),
...
func _build_character() -> void:
	...
	# ── Upper robe / mantle ─────────────────────────────────────────────────
	_char_box(_miriam, Vector3(0, 1.80, 0), Vector3(0.60, 0.65, 0.32),
		C(0.62, 0.46, 0.28))
	...
	# Drape over shoulder
	_char_box(_miriam, Vector3(-0.24, 2.20, 0.10), Vector3(0.16, 0.50, 0.12),
		C_HEAD_WRAP)
	...
	_char_box(_miriam, Vector3(0, 2.50, -0.26), Vector3(0.10, 0.025, 0.02),
...
func _run_cinematic() -> void:
	...
	cam_tween.parallel().tween_property(_camera, "rotation_degrees",
		Vector3(-12, -8, 0), 5.0)
	...
	sky_tween.tween_property(_sky_quad, "material_override:albedo_color",
		Color(1.0, 0.95, 0.80), 5.0)
	...
	wide.parallel().tween_property(_camera, "rotation_degrees",
		Vector3(-18, 5, 0), 3.0)
	await wide.finished
